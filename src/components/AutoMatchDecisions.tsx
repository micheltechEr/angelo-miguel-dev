...
export default function AutoMatchDecisions() {
  const { t } = useLanguage();
  const { visible, close } = useHashVisible("#auto-match-decisions");

  if (!visible) return null;

  return (
    <section id="auto-match-decisions" className="section-spacing px-6">
      <div className="mx-auto max-w-5xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h2 className="heading-section text-text-primary">
              {t.autoMatchDecisions.title}
            </h2>
            <p className="mt-2 text-sm text-text-secondary font-mono">
              {t.autoMatchDecisions.subtitle}
            </p>
          </div>
          <button
            onClick={close}
        ...
        <div className="mt-8 overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody>
              {t.autoMatchDecisions.decisions.map((row, i) => (
